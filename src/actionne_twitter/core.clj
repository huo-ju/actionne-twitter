...
(defn extratext [inputcontent]
  (clojure.string/join " " (map (fn [t]
                                  (if (= (type t) java.lang.String)
                                    t
                                    (if (= (type (:content t)) clojure.lang.PersistentVector) (apply str (:content t)) (extratext (first (:content t)))))) inputcontent)))
...
(defn parsenode [htmlnode]
  (let [id (-> (s/select (s/child (s/tag :li)) htmlnode) first :attrs :data-item-id)
    ...
    {:id id
     :object {:id id :favorite_count (Integer/parseInt favorite_count) :retweet_count (Integer/parseInt retweet_count) :created_at (Integer/parseInt created_at)
              :text (extratext text)
              :category (cond
     ...
     :original {:id_str (str id) :favorite_count favorite_count :retweet_count retweet_count :created_at created_at
                :text (extratext text)
    ...
    (if (= "web" (:method session))
      (let [uri (format "http://twitter.com/i/search/timeline?f=tweets&q=%s%%20include:nativeretweets%s" (URLEncoder/encode
                                                                                                          (if (not (nil? (:last_tweet_timestamp (:confirmed session))))
                                                                                                            ...
                                                                                                            (str search_term " until:" (:lastest_tweet env))) "UTF-8")
                   ...
                   (let [tweets (map (fn [input]
                                       (let [htmlnodes (hickory.core/as-hickory input)]
                                         (if (= (type htmlnodes) clojure.lang.PersistentArrayMap)
                                           (parsenode htmlnodes)))) (hickory.core/parse-fragment (val v)))]
                     (let [formattedtweets (remove nil? tweets)]
                       (log/info (str "tweets count:" (count formattedtweets)))
                       (let [first_tweet_timestamp (/ (inst-ms (.parse (java.text.SimpleDateFormat. "yyyy-MM-dd") (:first_tweet env))) 1000)
...
(defn backup [original]
  ...
  (let [urls (mediaurls original)]
    (let [result (mapv (fn [url]
                         (download url (str homedir "/data/" (:id original) "_" (#(nth % (dec (count %))) (clojure.string/split url #"/"))))) urls)]
  ...
  (let [creds (loadcredentials env)]
    (if (= (:screen_name creds) (:screen_name env))
      ...
      (do (if (and (some? (:consumer-key env)) (some? (:consumer-secret env)))
            ...
            (log/info (str "User Credentials found: " (:screen_name
